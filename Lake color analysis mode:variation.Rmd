...
# read_csv(dir(path = "data/lake_color_mod_variability_dw_newmode/", full.names = T, pattern = "*.csv")[1])
## load lake color data
dat = dir(path = "data/lake_color_mod_variability_dw_newmode/", full.names = T, pattern = "*.csv") %>% 
  map(read_csv, col_types = paste0("c", paste(rep("n", 9), collapse = ""))) %>% 
  reduce(bind_rows)

version = "b869faaa94414a9e1fe82687c0a5c04c"

dat %>% nrow()

print("how many centroids fall outside of the lake polygon?")
(dat %>% filter(is.na(dist2Shore)) %>% select(id) %>% distinct %>% nrow()) / (dat %>% select(id) %>% distinct %>% nrow)

print("Where are the lakes with centroid outside its polygons?")
lake_id_null_ct_dist2Shore = dat %>% filter(is.na(dist2Shore)) %>% select(id) %>% distinct
...
dat %>% filter(hue_count < 10) %>% select(id) %>% distinct() %>% nrow()
...
datFil %>% 
  gather(key = "variable", value = "mode", c(hue_mode, dw_mode, hue_count)) %>% 
  ggplot() +
...
output = dat %>% 
  select(id, hue_mode, hue_stdDev, hue_count, dw_mode, dw_stdDev, dist2Shore)
