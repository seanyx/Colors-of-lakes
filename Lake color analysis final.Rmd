...
save(grid_color_stats_sf, file = "outputs/grid_color_stats_sf.RData")
write_sf(grid_color_stats_sf, dsn = "outputs/figure1_global_lake_color_map_data.shp")
```
...
fitted_raw = rpart(mode ~ ., data = model_input, control = rpart.control(cp = 0.02))
...
nodeName = tibble(node_id = terminal_node_ids,
       node_name = c("wet and cool ",
                     "wet and warm",
                     "dry, high elevation, deep",
                     "dry, high elevation, shallow",
                     "dry, low elevation")) %>% 
...
swSAT = modelInput %>% 
  ...
  geom_vline(aes(xintercept = 18.98, lty = "x = 18.98ºC")) +
...
#   # geom_bar(aes(fill = `dw ≤ 526?`), position = "fill") +
#   geom_bar(aes(fill = `dw ≤ 526?`), position = "stack") +
...
#   labs(x = "Winter mean surface air temperature ºC",
#        y = "Lake count",
#        size = "Lake count",
#        color = "Mean depth (m)") +
...
climte_variable_grps = modelInput %>%
  mutate(
    ...
    `dw ≤ 526?` = cut(mode, breaks = c(min(mode), 526, max(mode)), labels = c("Blue", "Non-blue"), include.lowest = T),
    precip_grp = base::cut(total_precipitation_mean * 1000, breaks = c(0, 55, max(total_precipitation_mean * 1000)), labels = c("Low precip ≤55", "High precip > 55"), include.lowest = T),
    depth_grp = base::cut(depthMean, breaks = c(min(depthMean), 3.64, max(depthMean)), include.lowest = T))
...
climate_depth_blue_lake_ratio %>% ggsave(filename = "figs/summer_climate_depth_blue_lake_ratio_nobar2_deepLakes.png", width = 6.5, height = 3.5)


### previous figure but with finer color binning

modelInput %>% ggplot() + geom_histogram(aes(depthMean), color = "white") + scale_x_log10()
modelInput %>% filter(depthMean >= 20) %>% nrow() ## 646 lakes with mean depth ≥ 20 meter

climte_variable_grps = modelInput %>%
  mutate(
    summer_temp_grp = base::cut(summer_temperature_mean, breaks = seq(-5, 45, by = 2), include.lowest = T),
    `dw ≤ 526?` = cut(mode, breaks = c(min(mode), 526, max(mode)), labels = c("Blue", "Non-blue"), include.lowest = T),
    precip_grp = base::cut(total_precipitation_mean * 1000, breaks = c(0, 55, max(total_precipitation_mean * 1000)), labels = c("Low precip ≤55", "High precip > 55"), include.lowest = T),
    depth_grp = base::cut(depthMean, breaks = c(min(depthMean), 3.64, max(depthMean)), include.lowest = T))
...
climate_depth_blue_lake_ratio %>% ggsave(filename = "figs/summer_climate_depth_blue_lake_ratio_nobar2_deepLakes_temp2bin.png", width = 6.5, height = 3.5)

## relationship between blue lake ratio and other predictors
modelinput = modelInput %>% mutate(blue = mode <= 526, ice = winter_temperature_mean <= 0)
lfit = glm(formula = blue~lakeMeanOcc + mean_2m_air_temperature_mean + mean_2m_air_temperature_stdDev + depthMean + elevation + polygonArea + winter_temperature_mean + summer_temperature_mean + summer_total_precipitation_mean + ice, family = binomial(link = "logit"), data = modelinput)

summary(lfit)
gtsummary::tbl_regression(lfit)


modelInput %>% mutate(blue = mode <= 526, ice = winter_temperature_mean <= 0) %>% 
  ggplot() +
  geom_density(aes(x = winter_temperature_mean, fill = blue))
...
lm_eqn(modelInput %>% transmute(x = summer_temperature_mean, y = winter_temperature_mean))

swfit = lm(winter_temperature_mean~summer_temperature_mean, data = modelInput)
...
summer_winter_temperature = modelInput %>% 
  ...
  # geom_text(aes(x = 0, y = 30, label = "y = -45 + 2x, r2 = 0.60")) +
  coord_equal() +
  labs(x = "Summer mean surface air temperature ºC",
       y = "Winter mean surface air temperature ºC",
       color = "") +
...
summer_winter_temperature %>% 
  ggsave(filename = "figs/summer_winter_temperature_deep_lakes.pdf",
         width = 4,
         height = 8)
...
color_std_scatter = grid_color_stats_raw %>% 
  ggplot(aes(x = n, y = sd_mode)) +
  geom_point(aes(color = median_mode), alpha = 0.6) +
  scale_color_gradientn(limits = c(472, 588), colours = dw2FUI(472:588), na.value='#FFFFFF00', guide = guide_colorbar(title.position = "top")) +
  scale_x_log10() +
  # geom_smooth(alpha = 0.7, method = "loess") +
  labs(
    x = "Lake count",
...
cor(grid_color_stats_raw$n, grid_color_stats_raw$sd_mode, method = "pearson", use = "complete.obs")
