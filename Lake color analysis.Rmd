...
training_dtw = training_all %>% 
  select(-lake_id, -mean_hue, -sd_hue) %>% 
  as.matrix()

cluster_dtw <- tsclust(training_dtw, type = "h", k = 2L:4L, distance = "dtw_basic",
                      centroid = dba, control =hierarchical_control(method = "complete"),
                      preproc = NULL)

plot(cluster_dtw[[2]], type = "d", alpha = 0.2, color = "grey") 

this_clustering = cluster_dtw[[2]]
...
cluster_final <-tsclust(dtw_mat, type = "h", k = 3L, distance = "dtw_basic",
                     centroid = dba, control =hierarchical_control(method = "complete"),
                     preproc = NULL)
```
