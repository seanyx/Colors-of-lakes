...
all_lake_color %>% 
  ggsave(filename = "figures/all_lake_color_three_regions.png",
         width = 6,
         height = 7,
         dpi = 300)
...
lake_color_seasonality = doy_hue %>% 
  mutate(plotcolor = hsv(h = mean_hue, s = 1, v = 1)) %>% 
  ggplot() +
  geom_point(aes(x = doy, y = mean_hue, group = lake_id, color = plotcolor), alpha = 0.3, size = 0.05) +
...
lake_color_seasonality %>% 
  ggsave(
    filename = "figures/lake_color_seasonality.png",
    width = 7,
    height = 3,
    dpi = 300
  )
